...
export function SettingsPage() {
  const { license, setLicense, discordId, setDiscordId, discordUsername, setDiscordUsername, isLoggedIn, setIsLoggedIn } = useStore()
  ...
  const handleDiscordLogin = useCallback(async () => {
    if (!window.electronAPI) { setStatus('error'); setStatusMsg('Electron required for Discord login'); return }
    setLoading(true); setStatus('idle')
    const result = await window.electronAPI.discordLogin()
    setLoading(false)
    if (!result.success) { setStatus('error'); setStatusMsg(result.error || 'Login failed'); return }
    setDiscordId(result.discordId || '')
    setDiscordUsername(result.username || '')
    setIsLoggedIn(true)
    if (result.license) {
      const tierMap: Record<string, any> = { 'FREE': 'FREE', 'PRO': 'PRO', 'PREMIUM': 'PREMIUM' }
      setLicense({
        tier: tierMap[result.license.tier] || 'FREE',
        key: null,
        activated: true,
        expiryDate: result.license.expires ? new Date(result.license.expires) : null,
        discordId: result.discordId || null,
        username: result.username || null,
        lastVerified: Date.now(),
      })
    }
    setStatus('success'); setStatusMsg(`Logged in as ${result.username}`)
  }, [setLicense, setDiscordId, setDiscordUsername, setIsLoggedIn])

  const handleDiscordLogout = useCallback(async () => {
    if (window.electronAPI && discordId) {
      await window.electronAPI.discordLogout(discordId)
    }
    setDiscordId('')
    setDiscordUsername('')
    setIsLoggedIn(false)
  ...
  }, [discordId, setLicense, setDiscordId, setDiscordUsername, setIsLoggedIn])
      ...
      <div className="max-w-3xl mx-auto p-6 space-y-6 fade-in">
        ...
        <Section title="Account">
          <div className="rounded-2xl p-5" style={{ background: 'var(--bg-secondary)', border: '1px solid var(--border-subtle)' }}>
            ...
            {isLoggedIn ? (
              <div className="mb-3">
                <div className="flex items-center justify-between p-4 rounded-xl" style={{ background: 'var(--bg-tertiary)', border: '1px solid var(--border-subtle)' }}>
                  <div className="flex items-center gap-3">
                    <div className="w-9 h-9 rounded-full flex items-center justify-center" style={{ background: 'rgba(88,101,242,0.2)' }}>
                      <svg viewBox="0 0 24 24" className="w-5 h-5" fill="white"><path d="M20.317 4.37a19.791 19.791 0 0 0-4.885-1.515.074.074 0 0 0-.079.037c-.21.375-.444.864-.608 1.25a18.27 18.27 0 0 0-5.487 0 12.64 12.64 0 0 0-.617-1.25.077.077 0 0 0-.079-.037A19.736 19.736 0 0 0 3.677 4.37a.07.07 0 0 0-.032.027C.533 9.046-.32 13.58.099 18.057a.082.082 0 0 0 .031.057 19.9 19.9 0 0 0 5.993 3.03.078.078 0 0 0 .084-.028c.462-.63.874-1.295 1.226-1.994a.076.076 0 0 0-.041-.106 13.107 13.107 0 0 1-1.872-.892.077.077 0 0 1-.008-.128 10.2 10.2 0 0 0 .372-.292.074.074 0 0 1 .077-.01c3.928 1.793 8.18 1.793 12.062 0a.074.074 0 0 1 .078.01c.12.098.246.198.373.292a.077.077 0 0 1-.006.127 12.299 12.299 0 0 1-1.873.892.077.077 0 0 0-.041.107c.36.698.772 1.362 1.225 1.993a.076.076 0 0 0 .084.028 19.839 19.839 0 0 0 6.002-3.03.077.077 0 0 0 .032-.054c.5-5.177-.838-9.674-3.549-13.66a.061.061 0 0 0-.031-.03z"/></svg>
                    </div>
                    <div>
                      <div className="text-xs font-semibold text-white">{discordUsername || 'Discord User'}</div>
                      <div className="text-[9px] text-[var(--text-muted)] mt-0.5">ID: {discordId}</div>
                    </div>
                  </div>
                  <button onClick={handleDiscordLogout} className="text-[10px] px-3 py-1.5 rounded-lg transition-colors" style={{ color: '#999', background: 'var(--bg-secondary)' }}>
                    Logout
                  </button>
                </div>
              </div>
              ...
              <div className="mb-3">
                <button onClick={handleDiscordLogin} disabled={loading}
                  className="w-full flex items-center justify-center gap-2 px-5 py-3 rounded-xl text-xs font-semibold transition-all disabled:opacity-50"
                  style={{ background: '#5865F2', color: '#fff' }}>
                  {loading ? (
                    <><Loader2 className="w-3.5 h-3.5 animate-spin" /> Connecting...</>
                  ) : (
                    <><svg viewBox="0 0 24 24" className="w-4 h-4" fill="currentColor"><path d="M20.317 4.37a19.791 19.791 0 0 0-4.885-1.515.074.074 0 0 0-.079.037c-.21.375-.444.864-.608 1.25a18.27 18.27 0 0 0-5.487 0 12.64 12.64 0 0 0-.617-1.25.077.077 0 0 0-.079-.037A19.736 19.736 0 0 0 3.677 4.37a.07.07 0 0 0-.032.027C.533 9.046-.32 13.58.099 18.057a.082.082 0 0 0 .031.057 19.9 19.9 0 0 0 5.993 3.03.078.078 0 0 0 .084-.028c.462-.63.874-1.295 1.226-1.994a.076.076 0 0 0-.041-.106 13.107 13.107 0 0 1-1.872-.892.077.077 0 0 1-.008-.128 10.2 10.2 0 0 0 .372-.292.074.074 0 0 1 .077-.01c3.928 1.793 8.18 1.793 12.062 0a.074.074 0 0 1 .078.01c.12.098.246.198.373.292a.077.077 0 0 1-.006.127 12.299 12.299 0 0 1-1.873.892.077.077 0 0 0-.041.107c.36.698.772 1.362 1.225 1.993a.076.076 0 0 0 .084.028 19.839 19.839 0 0 0 6.002-3.03.077.077 0 0 0 .032-.054c.5-5.177-.838-9.674-3.549-13.66a.061.061 0 0 0-.031-.03z"/></svg> Login with Discord</>
                  )}
                </button>
                <p className="text-[9px] mt-2 text-center text-[var(--text-muted)]">Sign in with your Discord account to manage your license.</p>
              </div>
            )}

            {/* Manual key fallback */}
            <div className="mb-3">
              <div className="text-[10px] uppercase tracking-widest text-[var(--text-muted)] font-medium mb-2">Or activate with key</div>
              <div className="flex gap-2">
                <input type="text" placeholder="CHTX-XXXX-XXXX-XXXX" value={discordId} onChange={(e) => {
                ...
                <button onClick={handleVerifyDiscord} disabled={loading}
                  ...
                  <User className="w-3 h-3" />{loading ? '...' : 'Verify'}
                </button>
              </div>
            </div>
